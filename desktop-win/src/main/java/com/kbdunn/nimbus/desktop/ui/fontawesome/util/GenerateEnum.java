package com.kbdunn.nimbus.desktop.ui.fontawesome.util;
...
public class GenerateEnum {
	...
	private static final String ENUM_FILE = "C:\\Users\\kdunn\\Documents\\STS\\nimbus-desktop\\src\\main\\java\\com\\kbdunn\\nimbus\\desktop\\fontawesome\\FontAwesome.java";
	...
	private static Map<String, String> getIcons() {
		...
		try {
			...
			String cssClass, hex;
			for (Element e : html.getElementsByClass("row").last().children()) { // Cheatsheet has 2 .row elements - we want #2
				cssClass = e.ownText();
				hex = e.getElementsByTag("span").last()
						.ownText().replace("[&#x", "").replace(";]", "");
	...
	private static void writeEnumLines(BufferedWriter writer, Map<String, String> icons) throws IOException {
		...
		for (Entry<String, String> icon : icons.entrySet()) {
			outline = "\t" + icon.getKey().toUpperCase()
					.replace("FA-",  "").replace("-", "_")
					.replace("500PX", "FIVE_HUNDRED_PX");
			outline += "(\"\\u" + icon.getValue() + "\")";
