CREATE SCHEMA NIMBUS AUTHORIZATION DBA
	CREATE CACHED TABLE SYS (
		...
		NIMBUSPHERE_TOKEN VARCHAR(48),
		NIMBUSPHERE_ADDRESS VARCHAR(2083),
		IS_NIMBUSPHERE_VERIFIED BOOLEAN,
		IS_NIMBUSPHERE_DELETED BOOLEAN,
	...
	CREATE CACHED TABLE FILE (
		...
		NAME VARCHAR(255) GENERATED ALWAYS AS (REVERSE(SUBSTR(REVERSE(PATH), 0, INSTR(REVERSE(PATH), '/')))),
		PARENT_PATH VARCHAR(4096) GENERATED ALWAYS AS (REVERSE(SUBSTR(REVERSE(PATH), INSTR(REVERSE(PATH), '/')+1))),
		SIZE BIGINT, 
